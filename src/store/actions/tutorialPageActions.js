...
export const addComment = comment => async (firebase, firestore, dispatch) => {
  try {
    ...
    // Update the comment document with its own ID
    await firestore
      .collection("cl_comments")
      .doc(docref.id)
      .update({
    ...
    // Update tutorials collection if needed
    ...
    // Dispatch an action with the commentId
    dispatch({ type: actions.ADD_COMMENT_SUCCESS, payload: docref.id });
